...
(function($) {
    function Plot(target_, data_, options_) {
        ...
        var series = [],
            ...
            colors: ["#edc240", "#afd8f8", "#cb4b4b", "#4da74d", "#9440ed"],
                ...
                grid: {
                    ...
                    markingsLineWidth: 2,
                    markingsRoundDirection: "down", // "down", "up", or "none"
                    // interactive stuff
                    clickable: false,
                    hoverable: false,
        ...
        function drawGrid() {
            ...
            var round;
            if(options.grid.markingsRoundDirection == 'down') {
              round = {lower:Math.floor, upper:Math.floor};
            } else if(options.grid.markingsRoundDirection == 'up') {
              round = {lower:Math.floor, upper:Math.ceil};
            } else {
              var identity = function(x) { return x; }; // no change
              round = {lower:identity,   upper:identity};
            }
            ...
            if (options.grid.markings) {
                ...
                for (i = 0; i < markings.length; ++i) {
                    ...
                    if (xrange.from == xrange.to || yrange.from == yrange.to) {
                        ...
                        ctx.fillRect(round.lower(xrange.from),
                                     round.lower(yrange.to),
                                     round.lower(xrange.to - xrange.from),
                                     round.lower(yrange.from - yrange.to));
                    }
                }
            }
